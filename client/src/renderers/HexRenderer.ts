...
export class HexRenderer {
  ...
  private update() {
    ...
    // Ground tint shifts with the season.
    ...
    this.drawRipples();
    this.drawWeather(localWeather(weatherCells, this.playerPos));
    this.drawNight(timeOfDay);
    this.drawEmotes();
  }
  ...
  private drawHex(q: number, r: number, color: number) {
    ...
    this.graphics.stroke({ color: 0x000000, width: 1, alpha: 0.06 });
  }
  ...
  private drawWater(x: number, y: number) {
    ...
    this.graphics.poly(pts);
    this.graphics.fill({ color: 0x4a90c2, alpha: 0.9 });
  }
  ...
  private drawTree(e: Entity, x: number, y: number) {
    ...
    this.graphics.circle(x, y - 6 * s, HEX_SIZE * 0.62 * s);
    this.graphics.fill({ color: c });
    this.graphics.circle(x - 6 * s, y - 2 * s, HEX_SIZE * 0.4 * s);
    this.graphics.fill({ color: c });
    this.graphics.circle(x + 6 * s, y - 3 * s, HEX_SIZE * 0.36 * s);
    this.graphics.fill({ color: c });
  }
  ...
  private drawFlora(e: Entity, x: number, y: number) {
    ...
    if (e.species === 'grass' || e.species === 'fern' || e.species === 'clover') {
      ...
      for (let i = -1; i <= 1; i++) {
        ...
        this.graphics.lineTo(x + i * 4, y + 6 - 12 * s);
    ...
    // stem
    this.graphics.rect(x - 1, y - 2, 2, 10 * s);
    this.graphics.fill({ color: 0x4c8f52 });
    // bloom
    const c = petals[e.species || 'flower'] ?? 0xef6f9a;
    this.graphics.circle(x, y - 4 * s, HEX_SIZE * 0.28 * s);
    this.graphics.fill({ color: c });
    this.graphics.circle(x, y - 4 * s, HEX_SIZE * 0.1 * s);
  ...
  private drawFauna(e: Entity, x: number, y: number) {
    ...
    if (e.species === 'butterfly' || e.species === 'bird') {
      this.graphics.ellipse(x - 4, y, 5, 3);
      this.graphics.ellipse(x + 4, y, 5, 3);
      this.graphics.fill({ color: c });
      return;
    }
  ...
  private drawWeather(weather: Weather) {
    if (weather === 'rainy' || weather === 'snowy') {
      const target = weather === 'rainy' ? 120 : 80;
      while (this.weatherParticles.length < target) {
        this.weatherParticles.push({
          x: Math.random() * this.app.screen.width,
          y: Math.random() * this.app.screen.height,
          vx: weather === 'snowy' ? (Math.random() - 0.5) * 0.5 : -1,
          vy: weather === 'snowy' ? 1.2 : 8,
          life: 1,
        });
      }
    } else {
    ...
    for (const p of this.weatherParticles) {
      p.x += p.vx;
      p.y += p.vy;
      if (p.y > this.app.screen.height) { p.y = -5; p.x = Math.random() * this.app.screen.width; }
      // Drawn in screen space via overlay.
    }
    this.overlay.clear();
    for (const p of this.weatherParticles) {
      if (weather === 'snowy') {
        ...
        this.overlay.rect(p.x, p.y, 1.5, 7);
        this.overlay.fill({ color: 0xaaccee, alpha: 0.6 });
      }
    }
  }

  private drawNight(timeOfDay: number) {
    // Darkness peaks at midnight (0.0/1.0), clears at midday (0.5).
    const darkness = Math.max(0, Math.cos(timeOfDay * Math.PI * 2)) * 0.55;
    if (darkness <= 0.01) return;
    this.overlay.rect(0, 0, this.app.screen.width, this.app.screen.height);
    this.overlay.fill({ color: 0x101838, alpha: darkness });
  }
