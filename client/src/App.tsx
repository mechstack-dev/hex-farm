...
function App() {
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      switch (e.code) {
        ...
        case 'KeyH':
          if (altKey) {
            if (shiftKey) socket.emit('build_building', 'auto-harvester');
            else socket.emit('cook', 'goat-cheese-salad');
          } else {
            socket.emit('harvest');
        ...
        case 'KeyT':
          ...
          else if (shiftKey) socket.emit('build_building', 'mill');
          else socket.emit('build_building', 'seed-maker');
          break;
        case 'KeyY':
          if (altKey) socket.emit('build_building', 'mill');
          else socket.emit('teleport_home');
          break;
      }
  ...
  const categorizedInventory = () => {
    const categories: Record<string, {name: string, items: [string, number][]}> = {
        ...
        processed: { name: 'Processed', items: [] },
